...
{	
	...
	public function allbycar($carid)
	{
		return Sqlsrv::array(
			$this->conn->connect(),
			"SELECT A.ID
					...
					,A.INSURANCE
					-- ,A.CLAIMDATE
					,CONVERT(DATETIME, CONVERT(NVARCHAR(4),YEAR(A.CLAIMDATE)+543)+'/'+CONVERT(NVARCHAR(4),MONTH(A.CLAIMDATE))+'/'+CONVERT(NVARCHAR(4),DAY(A.CLAIMDATE))) [CLAIMDATE]
					,A.CLAIMDETAIL
					,IR.INSURANCEDES
					,CR.CARID
					,CR.REGCAR
			FROM CLAIM A
			LEFT JOIN MASTER_INSURANCE IR
			ON A.INSURANCE = IR.ID
			LEFT JOIN MASTER_CAR CR
			ON A.CARID = CR.CARID
			WHERE A.CARID = ?",
				[
					$carid
