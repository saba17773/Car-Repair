      ...
      <div class="modal-body">
        <form id="form_create" onsubmit="return submit_create_user()">
          ...
          <div class="form-group">
            <label for="inp_registerid" id="inp_registerid">รหัส</label>
            <input type="name" name="inp_registerid" id="inp_registerid" class="form-control" autocomplete="off" required>
          </div>
          <div class="form-group">
            <label for="inp_registername">ประเภทการจดทะเบียน</label>
            <input type="name" name="inp_registername" id="inp_registername" class="form-control" autocomplete="off" required>
          </div>
...
jQuery(document).ready(function($){
  ...
  else
  {
	...
	$('#edit').on('click', function(e) {
      var rowdata = row_selected("#gridreg");
      if (typeof rowdata !== 'undefined') {

        $('#modal_create').modal({backdrop: 'static'});
        $('input[name=form_type]').val('update');
        $('.modal-title').text('แก้ไขประเภทการจดทะเบียน');
        $('input[name=inp_registerid]').show();
        $('#inp_registerid').show();
        $('input[name=inp_registerid]').val(rowdata.ID);
        $('input[name=inp_registerid]').prop('readonly', true);
        $('input[name=inp_registername]').val(rowdata.REGISTERTYPE);
  ...
  $('#delete').on('click', function(e) {
    ...
    if (typeof rowdata !== 'undefined') {
    	$("#dialog").dialog({
      		buttons : {
        		...
        		gojax('post', '/api/v1/regi/delete', {id:rowdata.ID})
			    	.done(function(data) {
			    		if (data.status == 200)
						{
							gotify(data.message,"success");
			    			$('#dialog').dialog("close");
			          		$('#gridreg').jqxGrid('updatebounddata');
	...
	function modal_create_open(){
  	    ...
  	    $('.modal-title').text('เพิ่มประเภทการจดทะเบียน');
  	    $('input[name=form_type]').val('create');
        $('#inp_registerid').hide();
        $('input[name=inp_registerid]').hide().val('0');
	    ...
	    var dataAdapter = new $.jqx.dataAdapter({
		  ...
		  return $("#gridreg").jqxGrid({
	        ...
	        columns: [
	        	...
	        	{ text:"ประเภทการจดทะเบียน", datafield: "REGISTERTYPE"}
