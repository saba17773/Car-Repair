...
<div id=div_show>
  ...
  <form id= 'form_approved' >
    ...
    <input type="radio" name='ALL' id='YESALL'>อนมุัติทั้งหมด
    <input type="radio" name='ALL' id='NOALL'>ไม่อนมุัติทั้งหมด
    <input type="hidden" name="inp_year">
    <input type="hidden" name="inp_company">

    <br><br><button class="btn btn-primary btn-sm" id="Save">ยืนยัน </button>
  </form>
</div>  
      ...
      gojax('get', '/api/v1/all/repair', {id:IDR})
        .done(function(data) {
          $.each(data, function(index, val) {
              ...
              i++;
              totalprice = Number(totalprice) + Number(val.PRICE);
              var check7 = '';
              var check6 = '';
              if(val.STATUS == 7){
                  check7 = 'checked';
              ...
              if(i==1)
              {
                  ...
                  html += '<td><input type="radio" value="7"  name="' + i + "_status\"" + ' id="'  + i + "YES\"" + check7 +' checked> อนุมัติ</td>'
                  html += '<td><input type="radio" value="10"  name="' + i + "_status\"" + ' id="'  + i + "NO\"" + check6 +'> ไม่อนุมัติ</td>'
                  html += '</tr>';
